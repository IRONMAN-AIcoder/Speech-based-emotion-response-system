...
def _decode_and_transcribe(raw_path: str, wav_path: str) -> dict:
    """Blocking work for transcribe_and_detect, run in a worker thread (see
    call site) instead of directly on the event loop. librosa.load, Google's
    speech-recognition call, and the emotion model's predict() can each take
    several seconds - especially on a cold first request, when the emotion
    model is being loaded and compiled for the first time. Running any of
    that directly in an async endpoint blocks the entire event loop for that
    whole duration, which means the server can't even answer a /health check
    in the meantime - exactly what was causing Railway to see the app as
    unresponsive and kill/restart the container mid-request."""
    try:
...
async def transcribe_and_detect(audio: UploadFile = File(...)):
    ...
    try:
        return await asyncio.to_thread(_decode_and_transcribe, raw_path, wav_path)
    finally:
...
async def chat(audio: UploadFile = File(...), synthesize_speech: bool = Form(True)):
    ...
    reply = await asyncio.to_thread(ed.get_answer, text, emotion)
