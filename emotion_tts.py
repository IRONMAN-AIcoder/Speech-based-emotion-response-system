...
try:
    ...
    # No audio device / portaudio backend in this environment (e.g. a
    # headless container). Local playback just gets skipped, everything
    # else (feature extraction, synthesis, file output) still works.
    sd = None
    _SOUNDDEVICE_AVAILABLE = False

DEFAULT_EMOTION_MODEL_PATH = os.environ.get("EMOTION_MODEL_PATH", "/app/models/emotion_model.keras")
DEFAULT_TTS_WEIGHTS_DIR = os.environ.get("TTS_WEIGHTS_DIR", "/app/models")
...
class EmotionTTSEngine:
    def __init__(self, emotion_model_path=DEFAULT_EMOTION_MODEL_PATH, tts_model_path='emotion_tts_gan'):
        ...
        cond_path = os.path.join(DEFAULT_TTS_WEIGHTS_DIR, tts_model_path + '_conditioner.weights.h5')
        voc_path = os.path.join(DEFAULT_TTS_WEIGHTS_DIR, tts_model_path + '_vocoder.weights.h5')
    ...
    def synthesize(self, text, emotion='neutral', output_path='response.wav', play=False):
        print(f"\n{'='*50}")
        ...
        try:
            ...
            if play and _SOUNDDEVICE_AVAILABLE:
...
def speak_response(text, emotion='neutral', emotion_model_path=DEFAULT_EMOTION_MODEL_PATH,
                    output_path='response.wav', play=False):
    global _engine
    if _engine is None:
